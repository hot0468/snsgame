...
import { applyGradeToGain, rollActivityGrade, type ActivityGrade } from "./condition";
...
// 컨디션 판정은 systems/condition.ts(잎 모듈)가 소유한다 — coach가 같은 판정을 쓰는데
// offline↔coach가 서로를 부르면 순환이라 분리했다. 기존 import 경로 보존을 위해 그대로 재export한다.
// ⚠️ re-export만으로는 이 파일 안에서 못 쓴다 — 내부 사용분은 따로 import해야 한다.
export {
  activityFailChance,
  activityGreatChance,
  applyGradeToGain,
