...
d스토리 블로그(오버레이) — 게시글 2개, 둘 다 [잠김]
   ├ 글1  힌트 "F12"   → 주소창 ⋮ → `개발자 도구  F12` → Console 탭 → pw:itchlrh!
...
- 열려 있으면 그 아래 팝오버를 띄운다. 항목 **1개**: `개발자 도구  F12`
  ...
  - ⚠️ **"F12"는 라벨일 뿐이다 — 실제 F12 키를 바인딩하지 않는다**(사용자 확정).
    개발자 도구를 여는 경로는 이 항목 하나뿐이다. 글1의 힌트 "F12"는
    "F12를 눌러라"가 아니라 **"F12라고 적힌 걸 찾아라"**는 뜻이다.
    `window.addEventListener("keydown")`을 추가하지 마라 — 게임 밖 진짜 브라우저의
    개발자 도구가 열려 몰입이 깨지고, F12는 예약 단축키라 막지도 못한다.
