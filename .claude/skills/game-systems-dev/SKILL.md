...
## 노출 확률은 상수가 아니라 파이프라인이다

피드류(탐색 트윗 3칸 등)에 뜨는 확률을 조정할 땐 **상수만 바꾸고 끝내지 마라.** 확률 판정 뒤에 다른 경로가 끼어들면 선언값이 화면에 도달하기 전에 깎인다 — 실제로 고정 계정 30% 선언이 ①에코챔버 가로채기 →16%, ②특수 트윗 슬롯 덮어쓰기 →19%, ③대상 집합 미달 →26%로 세 겹으로 샜다(`systems/exploreSystem.ts`의 `maybeFixedAuthorTweet`·`specialSlot` 주석 참조).

- **판정 순서가 곧 우선순위다.** 지켜야 할 확률은 사슬 맨 앞에서 굴리고, 뒤 경로는 그 결과를 덮지 않게 한다.
- **선언값은 실측으로 검증하라.** n=수천 반복으로 실제 노출률을 재는 회귀 테스트를 남긴다(`feedTasteTrendRide.test.ts`의 ±4%p 대조 선례). 눈으로는 절대 못 잡는 종류의 누수다.
